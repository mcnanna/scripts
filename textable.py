...
class TexTable:
   ...
   def __init__(self, numcols, justs=None, fontsize=None, rotate=False, 
         tablewidth=None, tablenum=None, caption=None, label=None, comments=None, notes=None, doc=True):
   ...
   def add_data(self, data, label="", sigfigs=2, labeltype='cutin'):
      ...
      for datum in data:
         if type(datum) not in [types.ListType, numpy.ndarray, numpy.core.defchararray.chararray]: # Mitch: Added chararray
            if len(numpy.shape(data)) == 1: # Mitch: Deal with only adding a single row of data as one list
   ...
   def print_preamble(self, fp):
      ...
      if self.doc:
         fp.write("\\documentclass[twocolumns,tighten]{aastex61}\n") # Mitch
   ...
   def print_footer(self, fp): # Mitch: added all but \end{deluxetable} line
      if self.comments:
